...
const changePassword = async(req, res, next) => {
    ...
    try{
        ...
        const user = await User.findByIdAndUpdate(userId, {$inc: {attemptsToUpdatePassword: 1}}, {new: true, projection: {attemptsToUpdatePassword: 1, password: 1}})
        if(!user) throw new NotFound(`User with ID ${userId} not found`)
        else if(user.attemptsToUpdatePassword > 10) throw new Forbidden("You have made too many requests to update password. Please try again later")
